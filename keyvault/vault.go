...
func CreateVault(ctx context.Context, vaultName string) (keyvault.VaultsCreateOrUpdateFuture, error) {
	vaultsClient := getVaultsClient()
	tenantID, err := uuid.FromString(config.TenantID())
	if err != nil {
		return keyvault.VaultsCreateOrUpdateFuture{}, err
	}
...
func GetVault(ctx context.Context, vaultName string) (keyvault.Vault, error) {
	...
	return vaultsClient.Get(ctx, config.BaseGroupName(), vaultName)

}

// CreateVaultWithPolicies creates a new Vault with policies granting access to the specified user.
func CreateVaultWithPolicies(ctx context.Context, vaultName, userID string) (vault keyvault.VaultsCreateOrUpdateFuture, err error) {
	vaultsClient := getVaultsClient()
...
func SetVaultPermissions(ctx context.Context, vaultName string) (keyvault.VaultsCreateOrUpdateFuture, error) {
	vaultsClient := getVaultsClient()
...
func SetVaultPermissionsForDeployment(ctx context.Context, vaultName string) (keyvault.VaultsCreateOrUpdateFuture, error) {
	vaultsClient := getVaultsClient()
	tenantID, err := uuid.FromString(config.TenantID())
	if err != nil {
		return keyvault.VaultsCreateOrUpdateFuture{}, err
...
func GetVaults() {
	...
	for rgList, err := vaultsClient.ListByResourceGroupComplete(context.Background(), config.BaseGroupName(), nil); rgList.NotDone(); err = rgList.Next() {
...
func DeleteVault(ctx context.Context, vaultName string) (autorest.Response, error) {
	vaultsClient := getVaultsClient()
	return vaultsClient.Delete(ctx, config.BaseGroupName(), vaultName)
}
